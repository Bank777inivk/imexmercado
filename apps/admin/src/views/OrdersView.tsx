...
import { 
  ...
  ArrowClockwise, X, Truck, Check, Eye
    ...
    try {
      ...
      if (statusInput === 'Shipped' && selectedOrder.status !== 'Shipped') {
        ...
        await sendAutomatedEmail('order_shipped', selectedOrder.userEmail, {
          ...
          trackingLink: trackingLinkInput || ('https://www.google.com/search?q=' + encodeURIComponent(trackingNumInput))
        });
      }
    ...
    <div className="space-y-6 md:space-y-8 relative">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="text-left">
          ...
          <p className="text-sm font-medium text-gray-500">Gerez le flux de vos ventes en temps réel.</p>
        </div>
              ...
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 md:gap-8">
                ...
                <div className="flex items-center justify-between lg:justify-end gap-6 md:gap-10 border-t border-gray-50 pt-6 lg:border-0 lg:pt-0">
                  <div className="text-left lg:text-right">
                    <p className="text-[9px] text-gray-400 font-black uppercase tracking-[0.15em] mb-2">Etat</p>
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-full max-w-xl bg-white h-full shadow-2xl flex flex-col z-10 animate-in slide-in-from-right duration-300">
            ...
            <div className="p-6 border-b border-gray-100 flex items-center justify-between">
              <div className="text-left">
                ...
                <h3 className="text-lg font-black text-gray-900 mt-1">
                  ORD-{selectedOrder.id.replace('ORD-', '').slice(-6).toUpperCase()}
                </h3>
              </div>
              <button 
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8 text-left">
              {/* Customer Info */}
              <div className="space-y-4">
                <h4 className="text-xs font-black uppercase tracking-widest text-gray-400">Client & Livraison</h4>
                <div className="bg-gray-50/50 rounded-2xl p-5 border border-gray-100 space-y-3">
                  ...
                  <p className="text-xs text-gray-600">Email : <span className="font-bold text-gray-900">{selectedOrder.userEmail}</span></p>
                  <p className="text-xs text-gray-600">Tél : <span className="font-bold text-gray-900">{selectedOrder.userPhone || 'Non renseigné'}</span></p>
                  <hr className="border-gray-100" />
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1">Adresse de livraison</p>
                    <p className="text-xs text-gray-700 leading-relaxed font-medium">
                      {selectedOrder.shippingAddress?.address}<br />
                      {selectedOrder.shippingAddress?.zipCode} {selectedOrder.shippingAddress?.city}, {selectedOrder.shippingAddress?.country}
                    </p>
                  </div>
                </div>
              </div>
              ...
              {/* Items Table */}
              <div className="space-y-4">
                <h4 className="text-xs font-black uppercase tracking-widest text-gray-400">Articles Commandés</h4>
                <div className="space-y-3">
                  {selectedOrder.items?.map((item: any, idx: number) => (
                    <div key={`item-${idx}`} className="flex items-center gap-4 bg-white p-3 rounded-xl border border-gray-100">
                      {item.image && <img src={item.image} alt={item.name} className="w-12 h-12 object-cover rounded-lg border border-gray-100" />}
                      <div className="flex-grow min-w-0">
                        <p className="text-xs font-bold text-gray-900 truncate">{item.name}</p>
                        <p className="text-[10px] text-gray-500 font-bold uppercase tracking-wider">{item.price}€ x {item.quantity}</p>
                      </div>
                      <p className="text-xs font-black text-gray-900">{(item.price * item.quantity).toFixed(2)}€</p>
                    </div>
                  ))}
                  <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                    <span className="text-xs font-bold text-gray-500">Livraison :</span>
                    <span className="text-xs font-bold text-gray-900">{selectedOrder.shippingPrice ? `${selectedOrder.shippingPrice}€` : 'Gratuit'}</span>
                  </div>
                  <div className="flex items-center justify-between p-4 border-t border-gray-100">
                    <span className="text-sm font-black text-gray-900">Total :</span>
                    <span className="text-lg font-black text-primary">{selectedOrder.total?.toFixed(2)}€</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4 pt-4 border-t border-gray-100">
                <h4 className="text-xs font-black uppercase tracking-widest text-gray-400">Modifier l'État & Expédition</h4>
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    <label className="text-[9px] font-black uppercase tracking-widest text-gray-400 pl-1">Statut</label>
                    <select 
                      value={statusInput}
                    ...
                    >
                      ...
                      <option value="Awaiting Payment">Awaiting Payment (En attente de paiement)</option>
                  ...
                  {statusInput === 'Shipped' && (
                    <div className="p-4 bg-blue-50/50 border border-blue-100 rounded-2xl space-y-3 animate-in fade-in duration-200">
                      <p className="text-[10px] font-black uppercase tracking-widest text-blue-500">Informations de livraison</p>
                      
                      <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-1">
                          <label className="text-[8px] font-black uppercase tracking-widest text-gray-400 pl-0.5">Transporteur</label>
                          <input 
                            type="text" 
                        ...
                        <div className="space-y-1">
                          <label className="text-[8px] font-black uppercase tracking-widest text-gray-400 pl-0.5">Numéro de suivi</label>
                          <input 
                            type="text" 
                      ...
                      <div className="space-y-1">
                        <label className="text-[8px] font-black uppercase tracking-widest text-gray-400 pl-0.5">Lien de suivi colis</label>
                        <input 
                          type="url" 
                          placeholder="https://..."
