...
import { 
  Envelope, List, Calendar, User, 
  Trash, ArrowClockwise, PaperPlaneTilt, Eye, X, Check, FloppyDisk, Flame
} from '@phosphor-icons/react';
import { 
  subscribeToCollection, subscribeToDocument, setDocument, deleteDocument 
...
export function EmailsView() {
  const [activeTab, setActiveTab] = useState<'templates' | 'abandoned' | 'logs'>('templates');
  ...
  const [selectedTemplateKey, setSelectedTemplateKey] = useState<string>('order_confirmation');
  ...
  useEffect(() => {
    ...
    const unsubscribeTemplates = subscribeToDocument('settings', 'email_templates', (data) => {
      setTemplates(data);
    ...
    const unsubscribeCarts = subscribeToCollection('abandoned_carts', (data) => {
  ...
  const handleSendRelance = async (cart: any) => {
    if (!confirm(`Envoyer l'email de relance panier abandonné à ${cart.email} ?`)) return;
    try {
      ...
      if (success) {
        alert('📧 Relance panier abandonné simulée avec succès ! (Vérifiez les logs)');
      } else {
  ...
  const placeholders: Record<string, string[]> = {
    ...
    order_confirmation: ['{customer_name}', '{order_id}', '{order_items}', '{total_price}'],
    order_shipped: ['{customer_name}', '{order_id}', '{tracking_number}', '{tracking_link}']
  };
  ...
  return (
    <div className="space-y-6 md:space-y-8 text-left max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h2 className="text-2xl font-black text-gray-900 tracking-tight">Automations & Messages</h2>
          <p className="text-sm font-medium text-gray-500">Visualisez les logs, relancez les paniers perdus et éditez vos templates d'email.</p>
        </div>
        {!templates && !loading && (
          <button 
            disabled={seeding}
      ...
      {loading ? (
        <div className="py-20 text-center">
          ...
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Chargement des données...</p>
        </div>
      ) : (
        <>
          ...
          {activeTab === 'templates' && (
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="lg:col-span-3 space-y-3">
                <h4 className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">Choisir un type</h4>
                {templates && Object.keys(templates).filter(k => k !== 'id').map(key => (
                  <button
                    key={key}
                  ...
                  >
                    <Envelope size={20} weight={selectedTemplateKey === key ? 'fill' : 'regular'} />
                    <span className="text-xs font-bold truncate">{templates[key].name || key}</span>
                  </button>
                ))}
              </div>
              ...
              <div className="lg:col-span-9 grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="bg-white rounded-3xl border border-gray-150 p-6 md:p-8 space-y-6 flex flex-col justify-between">
                  <div className="space-y-6">
                    <div className="flex items-center justify-between border-b border-gray-100 pb-4">
                      <h3 className="font-black text-gray-900 text-sm uppercase tracking-wider">Configuration</h3>
                      <span className="text-[9px] bg-gray-100 text-gray-500 font-extrabold px-3 py-1 rounded-full uppercase">Édition</span>
                    </div>

                    <div className="space-y-2">
                      <label className="text-[9px] font-black uppercase tracking-widest text-gray-400 pl-1">Objet de l'email</label>
                      <input 
                        type="text"
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <label className="text-[9px] font-black uppercase tracking-widest text-gray-400 pl-1">Corps HTML</label>
                        <span className="text-[8px] font-bold text-gray-300 uppercase">Tags valides : {placeholders[selectedTemplateKey]?.join(', ')}</span>
                      </div>
                      <textarea
                        rows={12}
                ...
                <div className="bg-white rounded-3xl border border-gray-150 p-6 md:p-8 flex flex-col">
                  <div className="flex items-center justify-between border-b border-gray-100 pb-4 mb-4">
                    <h3 className="font-black text-gray-900 text-sm uppercase tracking-wider">Aperçu Visuel</h3>
                  ...
                  <div className="flex-1 bg-gray-50 rounded-2xl overflow-hidden border border-gray-100 h-[400px]">
                    <iframe
                      ...
                      srcDoc={editedBody
                        ...
                        .replaceAll('{cart_items}', `<table style="width:100%; border-collapse:collapse;"><tr style="border-bottom:1px solid #E5E7EB;"><td style="padding:8px 0; font-family:sans-serif; font-size:13px; font-weight:700;">Sony WH-1000XM5 x1</td><td style="text-align:right; font-family:sans-serif; font-size:13px; font-weight:900;">349.00€</td></tr></table>`)
                        .replaceAll('{order_items}', `<table style="width:100%; border-collapse:collapse;"><tr style="border-bottom:1px solid #E5E7EB;"><td style="padding:8px 0; font-family:sans-serif; font-size:13px; font-weight:700;">Sony WH-1000XM5 x1</td><td style="text-align:right; font-family:sans-serif; font-size:13px; font-weight:900;">349.00€</td></tr></table>`)
                      }
          ...
          {activeTab === 'abandoned' && (
            <div className="bg-white rounded-3xl border border-gray-150 overflow-hidden shadow-sm">
              <div className="p-6 md:p-8 border-b border-gray-100 flex items-center justify-between">
                <h3 className="font-black text-gray-900 text-sm uppercase tracking-wider">Paniers Abandonnés Actifs</h3>
              ...
              {abandonedCarts.length === 0 ? (
                <div className="py-20 text-center text-gray-400">
                  ...
                  <p className="text-xs font-bold">Aucun panier abandonné détecté pour le moment.</p>
                </div>
              ) : (
                      ...
                      {abandonedCarts.map(cart => (
                        <tr key={cart.id} className="hover:bg-gray-50/50 transition-colors">
                          <td className="py-4 px-6">
                            ...
                            <p className="text-[10px] text-gray-400 font-bold">{cart.email}</p>
                            {cart.phone && <p className="text-[9px] text-gray-400 font-medium">{cart.phone}</p>}
                          </td>
                          <td className="py-4 px-6">
                            <div className="space-y-1">
                              {cart.items?.map((item: any, idx: number) => (
                                <div key={idx} className="flex items-center gap-1.5">
                                  ...
                                  <span className="truncate max-w-[120px] inline-block text-[11px] font-bold text-gray-900">{item.name}</span>
                                </div>
                              ))}
                            </div>
                          </td>
                          ...
                          <td className="py-4 px-6 text-right">
                            <div className="flex items-center justify-end gap-2">
                              ...
                              >
                                <PaperPlaneTilt size={12} weight="bold" /> Relancer
                              </button>
                              <button
          ...
          {activeTab === 'logs' && (
            <div className="bg-white rounded-3xl border border-gray-150 overflow-hidden shadow-sm">
              <div className="p-6 md:p-8 border-b border-gray-100 flex items-center justify-between">
                <h3 className="font-black text-gray-900 text-sm uppercase tracking-wider">Historique des envois simulés</h3>
              ...
              {emailLogs.length === 0 ? (
                <div className="py-20 text-center text-gray-400">
                  <List size={40} className="mx-auto text-gray-200 mb-3" />
                  <p className="text-xs font-bold">Aucun email envoyé pour l'instant.</p>
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                        ...
                        <tr key={log.id || index} className="hover:bg-gray-50/50 transition-colors">
                          ...
                          <td className="py-4 px-6">
                            <span className="inline-flex items-center gap-1 text-[9px] text-green-600 font-black uppercase tracking-widest">
                              <span className="w-1.5 h-1.5 bg-green-500 rounded-full"></span> {log.status || 'Sent'}
                            </span>
                          </td>
                          <td className="py-4 px-6 text-right">
                            <button
      ...
      {viewingLog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-gray-900/40 backdrop-blur-sm" onClick={() => setViewingLog(null)} />
          <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl flex flex-col overflow-hidden max-h-[85vh] z-10 animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 border-b border-gray-100 flex items-center justify-between">
              <div>
                ...
                <h3 className="text-sm font-black text-gray-900">
                  Destinataire : <span className="text-gray-500 font-bold">{viewingLog.recipientEmail}</span>
                </h3>
              </div>
              <button 
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-gray-50 space-y-4">
              <div className="bg-white p-4 rounded-xl border border-gray-150">
                <p className="text-xs text-gray-500 font-medium">Objet : <span className="font-black text-gray-900">{viewingLog.subject}</span></p>
                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-tight mt-1">Date d'envoi : {new Date(viewingLog.sentAt).toLocaleString()}</p>
              </div>
