...
const StatCard = ({ label, value, diff, trend, icon: Icon, color }: any) => (
  <div className="bg-white p-5 md:p-8 rounded-3xl border border-gray-200 shadow-sm transition-all hover:shadow-md animate-in zoom-in-95 duration-500">
    <div className="flex items-start justify-between mb-3 md:mb-4">
      <div
        className={`p-2.5 md:p-3 rounded-2xl ${color} bg-opacity-10 text-opacity-100`}
      >
        <Icon size={22} weight="bold" className="md:size-24" />
      </div>
      <div
        className={`flex items-center gap-1 font-black text-[10px] md:text-xs ${trend === "up" ? "text-green-500" : "text-red-500"}`}
      >
        {trend === "up" ? (
    ...
    <div className="text-left">
      <p className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.15em] text-gray-400 mb-0.5 md:mb-1">
        {label}
      </p>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2 bg-white p-1.5 rounded-2xl border border-gray-100 shadow-sm w-fit font-black">
            <button className="px-4 py-2 text-[10px] font-black uppercase tracking-widest bg-gray-900 text-white rounded-xl">
              7 Jours
            </button>
          </div>
        </div>
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white p-6 md:p-8 rounded-3xl border border-gray-200 shadow-sm flex flex-col animate-in slide-in-from-right-4 duration-700">
              <h3 className="font-black text-xs md:text-sm text-gray-900 uppercase tracking-widest mb-6 md:mb-8 text-left">
                Dernières Commandes
              </h3>
                  ...
                  statsData.recentOrders.map((order) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        />
                        <div className="text-left">
                          <p className="text-xs font-black text-gray-900 truncate max-w-[100px] md:max-w-[120px]">
                            {order.userName}
                          </p>
