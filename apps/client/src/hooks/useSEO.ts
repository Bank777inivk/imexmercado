...
export function useSEO(pageKey?: 'home' | 'shop' | 'contact' | 'about' | 'faq', manualMetadata?: SEOMetadata) {
  ...
  useEffect(() => {
    ...
    if (manualMetadata?.title) {
      ...
      title = pageKey ? titles[pageKey] : 'ImexMercado';
    }
    ...
    if (manualMetadata?.description) {
      ...
      const descriptions = {
        ...
        contact: 'Besoin d\'aide ? Contactez notre service commercial ou technique par email.',
        about: 'En savoir plus sur IMEXSULTING Lda et nos centres de logistique en Europe.',
        faq: 'Retrouvez toutes les réponses concernant les livraisons, retours et garanties.'
      };
      description = pageKey ? descriptions[pageKey] : '';
    }
