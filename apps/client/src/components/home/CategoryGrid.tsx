...
export function CategoryGrid() {
  ...
  const [blogBanner, setBlogBanner] = useState({
    ...
    titlePT: "OS NOSSOS CONSELHOS E INSPIRAÇÕES",
    link: "/",
    isActive: true,
  });
  ...
  return (
    <div className="bg-bg-subtle py-6 md:py-8 w-full px-4 md:px-8 lg:px-12">
      <div className="w-full">
        ...
        {blogBanner.isActive && (
          <a
            href={localLink(blogBanner.link)}
          ...
          >
            <div className="flex items-center gap-3 md:gap-4">
              ...
              <span className="text-[11px] md:text-sm font-bold text-gray-800 uppercase tracking-tight">
                {currentLang.startsWith("pt")
                  ? blogBanner.titlePT || blogBanner.title
                  : blogBanner.title}
              </span>
            </div>
        ...
        <div className="grid grid-cols-3 md:grid-cols-3 lg:grid-cols-6 gap-y-12 md:gap-y-16 gap-x-2 md:gap-x-4 justify-items-center px-4 lg:px-0 mt-8 mb-8">
          {categories.map((cat: any, idx) => {
            const displayShort = currentLang.startsWith("pt")
              ? cat.shortPT ||
                t(`categories.${cat.short || cat.name}`, cat.short || cat.name)
              : cat.short || cat.name;
            return (
              <a
                href={localLink(cat.link)}
                key={idx}
              ...
              >
                <div className="reflection-container transition-all duration-700 group-hover:-translate-y-6">
                  <div className="w-[85px] h-[85px] md:w-32 md:h-32 flex items-center justify-center transition-all duration-700 group-hover:scale-110 group-hover:animate-float-slow">
                    <img
                      src={cat.image}
                      alt={cat.name}
                      ...
                      onError={(e) => {
                        e.currentTarget.src = `https://placehold.co/200x200/eeeeee/999999?text=${cat.short}`;
                      }}
                    />
                  </div>
                  {/* Reflection */}
