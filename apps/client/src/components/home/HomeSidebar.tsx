  ...
  const displayAds =
    ads.length > 0
      ...
      : [
          ...
          {
            image:
              "https://images.unsplash.com/photo-1524758631624-e2822e304c36?auto=format&fit=crop&q=80&w=400&h=600",
            title: "Office Style",
            subtitle: "Up to -30%",
            bgColor: "#0080001a",
      ...
      {displayAds.map((slide, idx) => (
        ...
        >
          ...
          <div
            ...
            style={{
              backgroundColor: slide.bgColor?.startsWith("#")
                ? slide.bgColor
                : undefined,
            }}
          >
            ...
            {!slide.bgColor?.startsWith("#") && (
              <div
                className={`absolute inset-0 ${slide.bgColor || "bg-black/20"} -z-10`}
              />
            )}
        ...
        <div className="flex gap-1">
          {chunks.length > 1 &&
            chunks.map((_, i) => (
              <div
                key={i}
                className={`w-1.5 h-0.5 transition-all ${index === i ? "w-3 bg-primary" : "bg-gray-300"}`}
              ></div>
            ))}
          {chunks.length <= 1 && (
            <>
            ...
            {chunk.map((product: any) => (
              ...
              >
                <div className="w-16 h-16 bg-gray-50 flex-shrink-0 flex items-center justify-center border border-gray-100 group-hover:border-primary transition-colors">
                  <img
                    src={product.image}
                    alt={isPT ? product.name : product.nameFR || product.name}
                ...
                <div className="flex flex-col text-left">
                  <h4 className="text-[11px] font-bold text-gray-800 line-clamp-2 leading-snug group-hover:text-primary transition-colors mb-1">
                    {isPT ? product.name : product.nameFR || product.name}
                  </h4>
        ...
        <div className="flex gap-1">
          {chunks.length > 1 &&
            chunks.map((_, i) => (
              <div
                key={i}
                className={`w-1.5 h-0.5 transition-all ${index === i ? "w-3 bg-secondary" : "bg-gray-300"}`}
              ></div>
            ))}
          {chunks.length <= 1 && (
            <>
  ...
  const displayStats =
    stats.length > 0
      ...
      : [
          ...
          { value: "50", label: "Produits", labelPT: "Produtos", emoji: "🗂️" },
          {
            value: "24h",
            label: "Traitement",
            labelPT: "Processamento",
            emoji: "⚡",
          },
          {
            value: "100%",
    ...
    <div className="bg-white p-5 border border-gray-200 mt-6 mb-10 text-left">
      ...
      <div className="space-y-3">
        {displayStats.map((stat, idx) => {
          const val = isPT ? stat.valuePT || stat.value : stat.value;
          const lbl = isPT ? stat.labelPT || stat.label : stat.label;
          return (
            <div
              key={idx}
              className="flex items-center gap-3 p-2 bg-gray-50 rounded-lg border border-transparent hover:border-primary transition-all group"
            >
      ...
      <div className="p-6 flex flex-col items-center text-center">
        ...
        <div className="mt-6 pt-4 border-t border-white/10 w-full text-center">
          ...
          <p className="text-[10px] text-gray-500 mt-1 uppercase tracking-tight">
            {testimonial.location}, PT —{" "}
            <span className="text-primary/70">{testimonial.category}</span>
          </p>
        </div>
      </div>
    </div>
...
export function HomeSidebar({
  ...
  const latestTitle = isPT
    ? s.latestTitlePT || s.latestTitle || "Últimos Produtos"
    : s.latestTitle || "Derniers Produits";
  const popularTitle = isPT
    ? s.popularTitlePT || s.popularTitle || "Populares"
    : s.popularTitle || "Populaires";
