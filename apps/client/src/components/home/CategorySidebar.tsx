...
const ICON_MAP: Record<string, any> = {
  ...
  "Telemóveis e Hi-Tech": DeviceMobile,
  "Maison & Décoration": House,
  "Casa e Decoração": House,
  "Meubles & Lampes": Couch,
  "Móveis & Candeeiros": Couch,
  Bricolage: Screwdriver,
  Bricolagem: Screwdriver,
  "Barbecues & Planchas": Tree,
  "Grelhadores & Planchas": Tree,
  "Piscines & Spas": SwimmingPool,
  "Piscina e Lazer": SwimmingPool,
...
export function CategorySidebar({ isOpen = true }: { isOpen?: boolean }) {
  ...
  const localizedCategories = useMemo(() => {
    return categories.map((cat) => {
      let name = isPT ? cat.namePT || cat.name : cat.name;
      if (name === cat.name) {
      ...
      if (isPT) {
        if (cat.subcategoriesPT) {
          subcategories = cat.subcategoriesPT;
  ...
  const handleCategoryClick = (catName: string) => {
    navigate(localLink(`/boutique?category=${encodeURIComponent(catName)}`));
  };
    ...
    <div className="w-[250px] bg-white border-x border-b border-gray-300 shadow-sm hidden lg:flex flex-col flex-shrink-0 relative z-40 h-full">
      <ul className="pt-2 flex-1 flex flex-col h-full">
          ...
          <li key={idx} className="relative group/item">
            ...
            >
              <div className="flex items-center gap-3">
                {(() => {
                  const Icon =
                    ICON_MAP[cat.name] || ICON_MAP[cat.short] || Package;
                  return (
                    <Icon
                      size={20}
                      className={`transition-colors ${activeSubmenu === idx ? "text-primary" : "text-gray-400 group-hover/item:text-primary"}`}
                    />
                  );
                })()}
              ...
              {cat.hasSubmenu && (
                ...
                >
                  <CaretRight
                    size={14}
                    weight="bold"
                    className={`transition-transform ${activeSubmenu === idx ? "rotate-90 text-primary" : "text-gray-400 group-hover/item:text-primary"}`}
                  />
                </button>
              )}
            </div>
                    ...
                    {cat.subcategories.map((sub: string, i: number) => (
                      <div
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          navigate(
                            `/boutique?category=${encodeURIComponent(cat.name)}&subcategory=${encodeURIComponent(sub)}`,
                          );
                        }}
                  ...
                  <div className="mt-auto pt-8">
                    <div className="bg-primary/5 rounded-2xl p-5 border border-primary/10">
                      <p className="text-[10px] font-black text-primary uppercase tracking-[0.2em] mb-1">
                        {isPT ? "Oferta Especial" : "Offre Spéciale"}
                      </p>
                      <p className="text-xs font-bold text-gray-900 leading-tight">
                        {isPT
                          ? `Aproveite -10% em toda a gama ${cat.name} este fim de semana!`
                          : `Profitez de -10% sur toute la gamme ${cat.name} ce weekend !`}
                      </p>
                    </div>
                  </div>
                </div>
        ...
        <li className="border-t border-gray-100 mt-0">
          <Link
            to={showAllLink}
            className="flex items-center px-4 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 hover:text-primary transition-colors bg-gray-50/50"
          >
            {isPT ? "Ver todo o catálogo +" : "Voir tout le catalogue +"}
          </Link>
        </li>
      </ul>
    </div>
  ...
  return (
    <div className="w-full h-full relative">
      {displayAds.map((ad, idx) => {
        const title = isPT ? ad.titlePT || ad.title : ad.title;
        const subtitle = isPT ? ad.subtitlePT || ad.subtitle : ad.subtitle;
        return (
          <div
            key={idx}
      ...
      {displayAds.length > 1 && (
        <div className="absolute bottom-2 right-2 flex gap-1">
          {displayAds.map((_, i) => (
            <div
              key={i}
              className={`w-1 h-1 rounded-full transition-all ${current === i ? "bg-primary w-2" : "bg-white/50"}`}
            />
          ))}
        </div>
      )}
