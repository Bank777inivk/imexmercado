...
export function TabbedProductSection({
  ...
  const tabs = [
    {
      id: "phares",
      label: isPT ? "⭐ Destaques" : "⭐ Phares",
      products: selections,
    },
    {
      id: "nouveautes",
      label: isPT ? "✨ Novidades" : "✨ Nouveautés",
      products: newArrivals,
    },
    {
      id: "promos",
      label: isPT ? "🔥 Promos" : "🔥 Promos",
      products: promotions,
    },
  ];
      ...
      <div className="w-full px-4 md:px-4 lg:px-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <h2 className="text-2xl font-bold text-text-primary">
            {isPT ? "As Nossas Seleções" : "Nos Sélections"}
          </h2>
        ...
        <div className="text-center mt-8">
          <a
            href={isPT ? "/pt/loja" : "/boutique"}
            className="inline-block border-2 border-primary text-primary font-bold px-10 py-2.5 rounded-full hover:bg-primary hover:text-white transition-all text-sm"
          >
            {isPT ? "Ver todos os produtos →" : "Voir tous les produits →"}
          </a>
        </div>
      </div>
    </section>
