    ...
    <div className="bg-bg-subtle pt-4 md:pt-6 pb-4">
      ...
      <div className="w-full px-2 md:px-4 lg:px-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-stretch lg:min-h-[400px] gap-0">
          ...
          <div className={`flex-1 flex flex-col md:flex-row gap-0 ${isSidebarOpen ? 'lg:pl-2' : ''} h-auto lg:min-h-[400px]`}>
            ...
            <div className="relative flex-[2] h-[250px] sm:h-[350px] lg:h-auto overflow-hidden group bg-black">
                ...
                {slides.map((slide) => (
                  <div key={slide.id} className="min-w-full h-full relative">
                    ...
                    <div className="absolute inset-0 flex flex-col justify-center items-start p-6 md:p-14 text-white max-w-sm md:max-w-lg">
                      <p className="text-[9px] md:text-sm font-black uppercase tracking-widest text-primary mb-2">{slide.title}</p>
              ...
              {/* Navigation Arrows */}
              <button onClick={prevSlide} className="hidden md:flex absolute left-4 top-1/2 -translate-y-1/2 p-2 bg-transparent text-white hover:bg-white/20 rounded-full opacity-0 group-hover:opacity-100 transition-opacity">
                <CaretLeft size={32} weight="bold" />
              </button>
              <button onClick={nextSlide} className="hidden md:flex absolute right-4 top-1/2 -translate-y-1/2 p-2 bg-transparent text-white hover:bg-white/20 rounded-full opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5 md:gap-2">
                {slides.map((_, idx) => (
                  <button key={idx} onClick={() => setCurrent(idx)} className={`h-1.5 md:h-1.5 rounded-full transition-all ${current === idx ? 'w-6 md:w-8 bg-white' : 'w-2 bg-white/50'}`} />
                ))}
              </div>
            </div>
