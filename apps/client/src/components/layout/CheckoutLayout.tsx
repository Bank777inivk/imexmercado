...
export function CheckoutLayout() {
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      if (
        securityRef.current &&
        !securityRef.current.contains(event.target as Node)
      ) {
        ...
        <div className="w-full max-w-[1600px] lg:px-12 mx-auto flex lg:flex-row items-center justify-between lg:justify-between grid grid-cols-3 lg:block lg:flex">
          ...
          <div className="flex justify-end relative" ref={securityRef}>
            ...
            >
              ...
              <span className="text-[9px] font-black uppercase tracking-tight lg:hidden">
                {t("header_secure_short")}
              </span>
              <span className="text-[9px] font-black uppercase tracking-widest hidden lg:inline">
                {t("header_secure_title")}
              </span>
            </button>

            {/* Downward Pop-over */}
            <div
              className={`absolute top-full right-0 mt-2 transition-all duration-300 origin-top-right z-30 ${showSecurityInfo ? "opacity-100 scale-100" : "opacity-0 scale-95 pointer-events-none"}`}
            >
              <div className="relative bg-white border border-success/20 shadow-xl shadow-success/10 rounded-xl px-4 py-3 min-w-[220px]">
                ...
                <div className="flex flex-col gap-1 text-left">
                  <div className="flex items-center gap-2 text-success">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest text-[#00A859]">
                      {t("header_secure_popup_title")}
                    </span>
                  </div>
                  <p className="text-[9px] text-gray-500 font-medium leading-relaxed">
                    {t("header_secure_popup_desc")}
                  </p>
                </div>
              </div>
            </div>
