...
// Custom hook to subscribe to legal page from Firestore with defaults fallback
function useLegalPage(key: string, defaultTitle: string, defaultDate: string) {
  ...
  useEffect(() => {
    const unsubscribe = subscribeToDocument('settings', 'legal_pages', (dbData: any) => {
      if (dbData && dbData[key]) {
        setData({
          title: dbData[key].title || defaultTitle,
          lastUpdated: dbData[key].lastUpdated || defaultDate,
          content: dbData[key].content || ''
        });
      }
    });
    return () => unsubscribe();
  }, [key, defaultTitle, defaultDate]);
...
export const LegalInfoPage = () => {
  const { title, lastUpdated, content } = useLegalPage('mentions', 'Mentions Légales', '12 Janvier 2024');
  return (
    <LegalLayout title={title} lastUpdated={lastUpdated} icon={<BookOpen size={36} weight="duotone" />}>
      {content ? (
        ...
        <>
          <section>
            ...
            <ul className="mt-4 space-y-2 pl-4 list-none text-left">
...
export const PrivacyPage = () => {
  const { title, lastUpdated, content } = useLegalPage('confidentialite', 'Politique de Confidentialité', '03 Mars 2024');
  return (
    <LegalLayout title={title} lastUpdated={lastUpdated} icon={<ShieldCheck size={36} weight="duotone" />}>
      {content ? (
        <div dangerouslySetInnerHTML={{ __html: content }} className="space-y-6" />
      ) : (
        <>
          ...
          <section>
            ...
            <ul className="list-disc pl-8 mt-4 space-y-2 marker:text-primary text-left">
...
export const CookiesPage = () => {
  const { title, lastUpdated, content } = useLegalPage('cookies', 'Politique des Cookies', '15 Février 2024');
  return (
    <LegalLayout title={title} lastUpdated={lastUpdated} icon={<Cookie size={36} weight="duotone" />}>
      {content ? (
        <div dangerouslySetInnerHTML={{ __html: content }} className="space-y-6" />
      ) : (
        <>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mt-6 md:mt-8 text-left">
...
export const CGVPage = () => {
  const { title, lastUpdated, content } = useLegalPage('cgv', 'Conditions Générales de Vente', '01 Janvier 2024');
  return (
    <LegalLayout title={title} lastUpdated={lastUpdated} icon={<FileText size={36} weight="duotone" />}>
      {content ? (
        <div dangerouslySetInnerHTML={{ __html: content }} className="space-y-6" />
      ) : (
        <>
...
export const ShippingInfoPage = () => {
  const { title, lastUpdated, content } = useLegalPage('livraison', 'Livraison & Expéditions', '18 Février 2024');
  return (
    <LegalLayout title={title} lastUpdated={lastUpdated} icon={<Truck size={36} weight="duotone" />}>
      {content ? (
        <div dangerouslySetInnerHTML={{ __html: content }} className="space-y-6" />
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 mt-8 md:mt-12 mb-8 md:mb-12 text-left">
...
export const ReturnsInfoPage = () => {
  const { title, lastUpdated, content } = useLegalPage('retours', 'Politique de Retours', '10 Décembre 2023');
  return (
    <LegalLayout title={title} lastUpdated={lastUpdated} icon={<ArrowUUpLeft size={36} weight="duotone" />}>
      {content ? (
        <div dangerouslySetInnerHTML={{ __html: content }} className="space-y-6" />
      ) : (
        <>
          ...
          <div className="bg-primary/5 border border-primary/20 rounded-2xl p-6 md:p-8 mt-6 md:mt-10 mb-8 md:mb-12 text-left">
          ...
          <section className="mb-12">
            ...
            <ol className="list-decimal pl-6 space-y-4 marker:font-black marker:text-gray-400 text-left">
